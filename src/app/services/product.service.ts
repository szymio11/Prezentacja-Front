...
export class ProductService {
  ...
  addProduct(product: ProductUpdate): Observable<ProductUpdate> {
    return this.http
      .post<ProductUpdate>(this.config.apiUrl + "product", product, httpOptions)
      .pipe
      //  catchError(this.handleError('addProduct', product))
      ();
  }
  getProducs(): Observable<Product[]> {
    return this.http.get<Product[]>(this.config.apiUrl + "product").pipe();
  }
  getByIdProduct(productId) {
    return this.http.get<Product>(
      this.config.apiUrl + "product" + "/" + productId
    );
  }
